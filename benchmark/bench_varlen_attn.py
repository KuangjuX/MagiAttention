...
def benchmark(
    ...
    # Packed KV for flash_attn style functions
    kv = torch.randn(
        seqlen,
        2,
        num_kv_heads,
...
# NEW: Dedicated benchmark function for zigzag_flex
def benchmark_zigzag_flex(
    f,
    process_group,
    dgrad_process_group,
    num_iter=100,
    ...
    # Config matches the other benchmark function
    seqlen = 1024 * 8
    num_heads = 32
    num_kv_heads = 8 # GQA is assumed
    head_dim = 128
    causal = True
    sm_margin = 0 # As per zigzag_flex's requirements
    ...
    k = torch.randn(
        seqlen, num_kv_heads, head_dim, device=device, dtype=dtype, requires_grad=True
    )
    v = torch.randn(
        seqlen, num_kv_heads, head_dim, device=device, dtype=dtype, requires_grad=True
    )
    ...
    # Use the same list of sequence length distributions
    cu_seqlens_list = [
        torch.tensor([0, 8192], device=device, dtype=torch.int32),
        torch.tensor([0, 256, 7648, 8192], device=device, dtype=torch.int32),
        torch.tensor([0, 4096, 8192], device=device, dtype=torch.int32),
    ...
    # This wrapper calls the function with the correct arguments for each cu_seqlens config
    def get_wrapper(i: int):
        cu_seqlens = cu_seqlens_list[i % len(cu_seqlens_list)]
        
        # Create ranges tensors from cu_seqlens for zigzag_flex
        ranges_tensor = torch.stack([cu_seqlens[:-1], cu_seqlens[1:]], dim=1).to(device)
        ...
        def func_wrapper():
            return f(
                ...
                dgrad_process_group=dgrad_process_group,
            )
        return func_wrapper
...
if __name__ == "__main__":
    ...
    # --- Benchmark for flash_attn, ring_flash_attn, etc. (Unchanged) ---
    for f, use_double_cu_seqlens in [
        ...
        if rank == 0:
            print(f"# {f.__name__}")
        f_compiled = torch.compile(f) if compile_func else f
        # Warmup run
        benchmark(
            f_compiled,
            use_double_cu_seqlens,
    ...
    # --- Benchmark for llama3 (Unchanged) ---
    for f, use_double_cu_seqlens in [
    ...
    # --- NEW: Benchmark for zigzag_flex_flash_attn ---
    torch.cuda.empty_cache()
    if rank == 0:
        print(f"# zigzag_flex_flash_attn_varlen_func")
    
    # Create process groups required by zigzag_flex
    # These groups will be used for the P2P communication within the function
    pg = dist.new_group()
    dgrad_pg = dist.new_group()

    f = zigzag_flex_flash_attn_varlen_func
    f_compiled = torch.compile(f) if compile_func else f
    
    # Warmup run
    benchmark_zigzag_flex(
        f_compiled,
        process_group=pg,
        dgrad_process_group=dgrad_pg,
        forward_only=forward_only,
        num_iter=num_iter,
        log=False,
    )
